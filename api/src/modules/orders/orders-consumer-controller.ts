...
export class OrdersConsumerController {
    ...
    async handleOrderCreated(
        ...
        this.logger.log(`Received order_created event for order ID: ${data.orderId}`);

        const pool = (this.databaseService as any).pool;
        ...
        try{
            ...
            this.logger.debug(`Order ID: ${data.orderId} processed successfully. Stock updated and cache invalidated.`);

        } catch (error) {
            await client.query('ROLLBACK');
            this.logger.error(`Error processing order ID: ${data.orderId}. Error: ${error}`);
